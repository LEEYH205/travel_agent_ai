...
class UserPreferences(BaseModel):
    ...
    interests: List[str] = Field(..., description="관심사 목록", min_items=1, max_items=20)
    pace: str = Field("balanced", description="여행 페이스", regex="^(relaxed|balanced|packed)$")
    budget_level: str = Field("mid", description="예산 수준", regex="^(low|mid|high)$")
    party: int = Field(2, description="여행 인원", ge=1, le=20)
    locale: str = Field("ko_KR", description="언어 설정")
    transport_mode: str = Field("walking", description="주요 교통수단", regex="^(walking|transit|driving)$")
...
class Transfer(BaseModel):
    ...
    mode: str = Field("walk", description="교통수단", regex="^(walk|transit|drive|bike)$")
...
class Tips(BaseModel):
    """현지 가이드 팁"""
    etiquette: List[str] = Field(default_factory=list, description="문화 및 예의", max_items=10)
    packing: List[str] = Field(default_factory=list, description="준비물", max_items=15)
    safety: List[str] = Field(default_factory=list, description="안전 및 주의사항", max_items=10)
    local_customs: Optional[List[str]] = Field(None, description="현지 관습", max_items=10)
    emergency_contacts: Optional[List[str]] = Field(None, description="긴급연락처", max_items=5)
...
class WeatherInfo(BaseModel):
    ...
    def calculate_fahrenheit(cls, v, values):
        if 'temp_c' in values:
            return round(values['temp_c'] * 9/5 + 32, 1)
        return v
...
class Itinerary(BaseModel):
    ...
    days: List[DayPlan] = Field(..., description="일별 일정", min_items=1, max_items=30)
